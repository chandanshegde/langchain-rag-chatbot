...
| **Agentic AI**       | LangChain + Gemini | ReAct loop with dynamic tool discovery.        |
...
- **Production-Grade Session Management:** The Orchestrator integrates seamlessly with **Redis**. Each UI interaction passes a unique `session_id`. The backend stores and retrieves the last several conversational exchanges from the Redis cache in sub-milliseconds, allowing the agent to remember context flawlessly without blowing out prompt limits. The frontend also implements session switching natively.
...
- **Transparency:** Collapsible "Reasoning Chain" UI that shows exactly how the AI used MCP tools.
